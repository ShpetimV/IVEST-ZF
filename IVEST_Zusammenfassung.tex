...
\linespread{0.97}
\setlist{nosep,leftmargin=1.0em,topsep=0pt,partopsep=0pt,itemsep=0.3pt,parsep=0pt}
...
\definecolor{tbg}{HTML}{f2f2f2}

\newcommand{\sechead}[1]{\par\vspace{2.5pt}\noindent{\colorbox{cA}{\color{white}\bfseries\,#1\,}}\par\vspace{1.2pt}}
...
\newcommand{\hl}[1]{\colorbox{yellow!45}{#1}} 
...
\newtcolorbox{tipbox}{colback=cWarn,colframe=red!55!black,boxrule=0.3pt,left=2.5pt,right=2.5pt,top=1.3pt,bottom=1.3pt,arc=1pt,before skip=2pt,after skip=2pt}
...
\K{Statische Rechnung} (Zeitpunkt/Stichtag). Zeigt Bestände, aber \emph{nicht die Ursache} für Veränderung der flüssigen Mittel oder des Gewinns.\\
...
EK ist die \emph{Ausgleichsgrösse} $\Rightarrow$ Bilanz geht \emph{immer} auf. Aktiven sortiert nach Liquidität (oben am flüssigsten), Passiven nach Fälligkeit.
 
{\centering\includegraphics[width=\linewidth]{images/bilanz.png}\par}
...
\sechead{Die drei Jahresrechnungen}
\begin{tabular}{@{}l@{~}l@{}}
\K{Bilanz} & statisch -- Bestände (1.\ JR)\\
\K{Erfolgsr.} & dynamisch -- \emph{warum} $\Delta$EK? (2.\ JR)\\
\K{Geldflussr.} & dynamisch -- \emph{warum} $\Delta$flüss.\ Mittel? (3.\ JR)\\
\end{tabular}\\
Erfolgsr.\ erklärt die Zunahme EK (Gewinn), Geldflussr.\ die Veränderung der flüssigen Mittel. Beide ``füttern'' die Schlussbilanz.
 
\sechead{Erfolgsrechnung (mehrstufig)}
Stellt Aufwand \& Ertrag einer Periode gegenüber
$\Rightarrow$ Gewinn (Ertrag$>$Aufwand) oder Verlust. Der Gewinn fliesst ins EK der Bilanz (= Brücke).\\
\begin{tabular}{@{}r@{~}l@{}}
...
\end{tabular}\\
``Earnings Before \dots'': von oben nach unten immer mehr Kostenarten dazu. EBITDA = rein operative Ertragskraft (gut für Firmenvergleich, unabhängig von Finanzierung/Steuern/Abschr.).
 
{\centering\includegraphics[width=\linewidth]{images/erfolgsrechnung.png}\par}
...
\K{Ertrag/Aufwand $\neq$ Ein-/Auszahlung} (Kernkonzept!): eine Ein-/Auszahlung ist \emph{immer} Geldfluss; ein Aufwand/Ertrag muss \emph{keiner} sein.
\begin{itemize}
\item Abschreibung \& Bildung Rückstellung: Aufwand \emph{ohne} Auszahlung (nicht liquiditätswirksam).
\item Umsatz auf Rechnung: Ertrag, Geld erst bei Zahlung $\to$ landet in Ford.\ L+L.
\item Einkauf auf Rechnung: Aufwand, Auszahlung erst später $\to$ Verb.\ L+L.
\item Wertzunahme Wertschrift: Ertrag ohne Einzahlung (erst bei Verkauf).
\end{itemize}
\R{Folge:} Firma kann profitabel \emph{und} illiquide sein (Gewinn, aber alles auf Rechnung, kein Cash). Genau das deckt die Geldflussr.\ auf.
 
\sechead{3 Bereiche der Geldflussr.}
\begin{itemize}
\item \K{CFO} (Betrieb): laufendes Geschäft. Basis = Erfolgsrechnung.
\item \K{CFI} (Investition): Kauf Anlagen $-$ / Verkauf Anlagen $+$.
\item \K{CFF} (Finanzierung): Aufnahme FK/EK $+$ / Rückzahlung \& Dividende $-$.
\end{itemize}
\fb{CFO+CFI+CFF=\Delta\,\text{flüssige Mittel}}\\
\K{Darstellung}: innere Spalte = Einzelposten, äussere Spalte = \emph{Subtotal pro Bereich}. Die drei Subtotale werden \emph{erst in der Schlusszeile} addiert (nicht laufend!).\\
\R{Drei-Vorzeichen-Muster} (Firmenphase ablesen):
\begin{itemize}
\item Reif (Nestlé): CFO\,$+$, CFI\,$-$, CFF\,$-$.
\item Startup: CFO\,$-$, CFI\,$-$, CFF\,$+$ (lebt von Investorengeld).
\end{itemize}
 
\sechead{Direkte Cashflow-Berechnung}
Pro liq.-wirksamem Erfolgskonto das \emph{Bilanz-Gegenkonto} kombinieren, um vom gebuchten auf den gezahlten Betrag zu kommen.\\
\K{Vorgehen}: (1) liq.-wirksame Erfolgskonti (Abschr.\ ist \emph{nie} lw); (2) Gegenkonto suchen; (3) korrigieren; (4) summieren (Ertrag $+$, Aufwand $-$).\\
\K{Vorzeichenregel} (\hl{Aktiven \& Passiven, Zu- UND Abnahme}):
\begin{tabular}{@{}l@{}}
$+$Saldo Ertrag\ \ $-$Zunahme Aktiven\ \ $=$ lw Ertrag\\
$+$Saldo Ertrag\ \ $+$Abnahme Aktiven\ \ $=$ lw Ertrag\\
$+$Saldo Ertrag\ \ $+$Zunahme Passiven $=$ lw Ertrag\\
$+$Saldo Ertrag\ \ $-$Abnahme Passiven $=$ lw Ertrag\\
$-$Saldo Aufwand $-$Zunahme Aktiven\ \ $=$ lw Aufw.\\
$-$Saldo Aufwand $+$Abnahme Aktiven\ \ $=$ lw Aufw.\\
$-$Saldo Aufwand $+$Zunahme Passiven $=$ lw Aufw.\\
$-$Saldo Aufwand $-$Abnahme Passiven $=$ lw Aufw.\\
\end{tabular}\\
\K{Merkregel (Intuition)}:
\begin{itemize}
\item \K{Aktiven (UV)}: \emph{Zunahme} bindet Geld $\Rightarrow$ entgegengesetztes Vorzeichen. \emph{Abnahme} setzt Geld frei $\Rightarrow$ gleiches Vorzeichen.
\item \K{Passiven (kurzfr.\ FK)}: \emph{Zunahme} schiebt Zahlung auf $\Rightarrow$ gleiches Vorzeichen. \emph{Abnahme} = Zahlung jetzt $\Rightarrow$ entgegengesetzt.
\end{itemize}
\hl{Kurz}: Aktiven \emph{umgekehrt}, Passiven \emph{gleichgerichtet}.
 
{\centering\includegraphics[width=\linewidth]{images/geldflussrechnung_direkt.png}\par}
 
\begin{exbox}
\textbf{Zahlung von Kunden}: Warenertrag 1'200; Ford.\ L+L $38{\to}58$ (Aktive $\uparrow20$).\\
$1'200-20=\R{1'180}$ (20 noch offen).\\[2pt]
\textbf{Zahlung an Lieferanten}: Warenaufw.\ $-660$; Vorrat $65{\to}35$ (Aktive $\downarrow30$); Verb.\ $45{\to}55$ (Passive $\uparrow10$).\\
$-660+30+10=\R{-620}$.\\[2pt]
Personal $-300$, übriger Aufw.\ $-140$ (kein Gegenkonto, bar).\\
\R{CFO direkt $=1180-620-300-140=+120$}
\end{exbox}
 
\sechead{Indirekte Cashflow-Berechnung}
Start beim \K{Reingewinn}, korrigiere um alles nicht-Geldwirksame. (Methode aller echten Geschäftsberichte.)\\
\fb{CF=RG+\text{Abschr.}\pm\text{nicht-lw}\mp\Delta\text{Akt.}\pm\Delta\text{Pass.}}\\
$RG$ = Reingewinn; $\Delta\text{Akt.}$ / $\Delta\text{Pass.}$ = Veränderung NUV (UV \& kurzfr.\ FK, ohne flüss.\ Mittel; plus langfr.\ Rückstellungen).\\
\K{Vorzeichen} (gleich wie direkt):
\begin{itemize}
\item $-$Zunahme / $+$Abnahme: Ford.\ L+L, Vorräte, unfert./fert.\ Erzeugnisse, ARA (TA).
\item $+$Zunahme / $-$Abnahme: Verbindlichkeiten, PRA (TP).
\end{itemize}
\hl{Aktiven umgekehrt, Passiven gleich} -- gleiche Regel wie oben.
 
{\centering\includegraphics[width=\linewidth]{images/geldflussrechnung_indirekt.png}\par}
 
\begin{exbox}
\textbf{Indirekt} (gleiche Firma): RG 30; $+$Abschr.\ 70; $-$Zunahme Ford.\ 20; $+$Abnahme Vorräte 30; $+$Zunahme Verb.\ 10.\\
$30+70-20+30+10=\R{120}$ ($=$ direkter CFO! Immer Kontrolle.)
\end{exbox}
 
\sechead{Investition \& Finanzierung}
\K{CFI}: Kauf Anlagen ($-$, Investition) \& Verkauf zum Buchwert ($+$, Desinvestition).
\K{CFF}: AK-Erhöhung, Hypothek-Aufnahme ($+$); Dividende, Rückzahlung ($-$).
\begin{exbox}
CFI: Kauf Liegenschaft $-200$, Verkauf Fahrzeug $+8$ $\Rightarrow\R{-192}$.\\
CFF: AK $+50$, Hypothek $+35$, Dividende $-20$ $\Rightarrow\R{+65}$.
\end{exbox}
 
\sechead{Lückentechnik (Anlagespiegel)}
Investitionen sind oft \emph{nicht direkt} gegeben $\Rightarrow$ als einzige Unbekannte herausrechnen. Anlagekonto bewegt sich nur durch 3 Vorgänge:\\
\fb{AB-\text{Abschr.}+\text{Inv.}-\text{Desinv.}=SB}\\
$AB/SB$ aus Bilanz, Abschr.\ aus ER, Desinv.\ aus Aufgabentext. \emph{Inv.}\ bleibt $\Rightarrow$ auflösen.
\begin{exbox}
$215-50+?-45=190 \Rightarrow ?=\R{70}$\\
Ohne Kauf wäre SB $=120$, tatsächlich $190$ $\Rightarrow$ Differenz $+70$ \emph{muss} ein Kauf sein.\\[2pt]
\R{Cash}: Inv.\ $70 \to -70$ in CFI; Verkauf $45 \to +45$ in CFI; Abschr.\ \emph{nicht} (nicht lw, in CFO indirekt $+$).
\end{exbox}
 
\sechead{Lückentechnik (Dividenden/Reserven)}
Bilanzgewinn Vorjahr $\to$ Reserven, Dividende, oder Vortrag (= alle möglichen Wege).
\begin{exbox}
\textbf{Aufg.\ 22}: Reserve $70{\to}90$ ($+20$). Zugewiesen Div.\ 8, Saldo Div.-Konto 1.1.\ $=5$ $\Rightarrow$ \R{ausbezahlt $8+5=13$}.\\
Vortrag: $30-20-8=2$; Verlust $20\_6$ von $20$ $\Rightarrow 2-20=\R{-18}$.\\
\R{Falle}: zugewiesene Div.\ ($8$) $\neq$ ausbezahlte Div.\ ($13$)!
\end{exbox}
 
\sechead{Gesamtrechnung \& Liquiditätsnachweis}
\begin{exbox}
CFO $+120$, CFI $-192$, CFF $+65$ $\Rightarrow\R{-7}$.\\
\K{Liquiditätsnachweis}: flüss.\ Mittel $12{\to}5=-7$ \checkmark\\
\R{Eingebaute Prüfsumme}: Summe der 3 Bereiche \emph{muss} der Bilanz-$\Delta$ der flüssigen Mittel entsprechen, sonst Fehler.
\end{exbox}
 
\begin{tipbox}
\textbf{Häufige Vorzeichen-Fehler}\\
\R{$\times$} Subtotale laufend statt blockweise addieren.\\
\R{$\times$} Aktiven-Zunahme \emph{positiv} eingesetzt (falsch: bindet Cash $\to$ negativ).\\
\R{$\times$} Abschreibung in CFI statt CFO (sie ist nicht lw!).\\
\R{$\times$} Zugewiesene statt ausbezahlte Dividende verwendet.
\end{tipbox}

\sechead{Finanzierung — Grundlagen}
{\centering\includegraphics[width=\linewidth]{images/finanzierung.png}\par}
...
Credit Spread = Risikozuschlag (aus Rating).
...
\K{Eigenkapitalkosten (CAPM - Capital Asset Pricing Model)} = verlangte Mindestrendite (RRR);
...
$\beta$ = systematisches (Markt-)Risiko (meist 0.5–2;
...
\K{WACC (Weighted Average Cost of Capital)} = $\varnothing$ gewichtete Kapitalkosten = Diskontsatz DCF = geforderte Mindestrendite:\\
\fb{\text{WACC}=k_{EK}\tfrac{EK}{GK}+k_{FK}\tfrac{FK}{GK}}\\
\fb{\text{WACC}_{tax}=k_{EK}\tfrac{EK}{GK}+k_{FK}\tfrac{FK}{GK}(1{-}t)}\\
$t$ = Steuersatz.
\K{Tax-Shield}: Steuerersparnis durch abzugsfähige FK-Zinsen $\Rightarrow$ WACC$_{tax}$ tiefer.
...
\K{Barwert (PV)}:\\
\fb{PV=\dfrac{CF_n}{(1+R)^n}=CF_n\cdot\text{AbF}} (AbF$_{R,n}=\frac{1}{(1+R)^n}$ für einzelnes Jahr)\\
\K{Annuität/Rente} (konstanter CF): \fb{PV=CF\cdot\text{RBF}_{R,T}}\\
$R$ = Diskontsatz = WACC.
RBF$_{R,T} =\sum_{n=1}^{T}$AbF (ab J1);
ewig RBF$_\infty=\tfrac1R$; mit Wachstum: RBF $=\tfrac{1}{R-g}$.\\
\K{Kapitalwert (NPV / NBW)}: Barwert aller Cashflows (Einnahmen $-$ Kosten).\\
\fb{\text{NPV}=\sum_{n=1}^{T}\dfrac{CF_n}{(1+R)^n}-K_0}\\
...
\end{exbox}
\K{Net Present Costs (NPC)}: Barwert aller Kosten eines Systems über die Lebensdauer ($+K_0$) (Systemvergleich, z.B.\ Heizung; Restwert im letzten Jahr abziehen).\\
\fb{\text{NPC}=\sum_{n=1}^{T}\dfrac{\text{Jährliche Kosten}}{(1+R)^n}+K_0}\\
...
\fb{IRR=i_1+\dfrac{\text{RBF}_{i_1}-\text{RBF}}{\text{RBF}_{i_1}-\text{RBF}_{i_2}}}\\
...
\fb{CF_1 = \text{Annuität} = \dfrac{K_0}{\text{RBF}}} (z.B.\ Leasingrate).\\
...
\begin{tabular}{|>{\raggedright\arraybackslash}p{3.1cm}|>{\raggedright\arraybackslash}p{3.1cm}|}
\hline
\rowcolor{tbg}
\textbf{Investitions\-rechnung} & \textbf{Unternehmens\-bewertung} \\ \hline
\multicolumn{2}{|l|}{\cellcolor{tbg}\textbf{Zentrale Frage}} \\ \hline
\textbf{Lohnt sich die Investition?} & Was ist der Wert eines Unternehmens? \\ \hline
\multicolumn{2}{|l|}{\cellcolor{tbg}\textbf{Bewertungsobjekt}} \\ \hline
Einzelne \K{Investitions\-projekte}, d.h. \textbf{ausgewählte Positionen} der Aktivseite einer Bilanz (z.\,B. Maschinen, Fahrzeuge, IT-Infrastruktur, Immobilien) & Ganze Unternehmen oder Unternehmensteile (z.\,B. Tochtergesellschaften) \\ \hline
\multicolumn{2}{|l|}{\cellcolor{tbg}\textbf{Anzahl Bewertungsobjekte}} \\ \hline
Vergleich mehrerer Investitions\-alternativen & Einzelnes Bewertungsobjekt \\ \hline
\multicolumn{2}{|l|}{\cellcolor{tbg}\textbf{Bekanntheit der Daten}} \\ \hline
Investitionsbetrag ist normalerweise \textbf{bekannt} & Unternehmenswert ist unbekannt und soll ermittelt werden \\ \hline
\multicolumn{2}{|l|}{\cellcolor{tbg}\textbf{Betrachtete Periode}} \\ \hline
Nutzungsdauer des Investitions\-objekts (\textbf{endliche Dauer}) & Unendlichkeit (Annahme: Unternehmen existiert in alle Ewigkeit), wobei mit einem Endwert \textbf{(Terminal Value)} gerechnet wird, der alle Erträge bis in die Ewigkeit einschliesst \\ \hline
\end{tabular}
...
\K{Equity} = Netto-UW (nur EK), Diskont $k_{EK}$.\\
\K{Substanzwert} (gegenwartsorientiert, oft Wertuntergrenze):

{\centering\includegraphics[width=\linewidth]{images/substanzwert.png}\par}

\K{Stille Res.} ($EK_{\text{intern}} - EK_{\text{extern}}$): Unterbewertung Aktiven o. Überbewertung Passiven. \emph{Extern} (Gläubigerschutz $\to$ schlechtere Lage) vs.\ \emph{Intern} (Entscheidung $\to$ realer Wert).\\
$+$ anlageintensive Branchen/Banken;
$-$ vernachlässigt Zukunft \& Risiko, untauglich für wachstumsstarke Firmen.

\begin{exbox}
\textbf{Substanzwert} (Mio): Vorräte Bilanz 300/Markt 320 ($+$20), AV 1'200/1'400 ($+$200), FK 950. Brutto-SW $=1'700+220=1'920$. Netto-SW $=1'920-950=\R{970}$ ($=EK\,750 + 220$).
\end{exbox}
...
NOPAT = Betriebsgewinn nach approx.\ Steuern, vor Zins.
FCF \emph{vor} Zinsen (steht EK $+$ FK-Gebern zur Verfügung, Entity).
\begin{exbox}
...
höherer Verschuldungsgrad erhöht FK-Kosten.\\[3pt]
\end{tcolorbox}
...
\sechead{Faktoren-Tabellen}
\vspace{0.5cm}
\noindent
\begin{minipage}{19.8cm}
  \centering
  \K{Abzinsungsfaktoren (AbF) Tabelle A}\\[4pt]
  \includegraphics[width=\linewidth]{images/abzinsungsfaktoren.png}\par\vspace{1cm}
  \K{Rentenbarwertfaktoren (RBF) Tabelle B}\\[4pt]
  \includegraphics[width=\linewidth]{images/rentenbarwertfaktoren.png}
\end{minipage}
\end{document}