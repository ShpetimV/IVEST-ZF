...
\K{Indirekte Berechnung} (Ausgang Reingewinn):\\
\begin{tabular}{@{}r@{~}l@{}}
 & Reingewinn\\
...
\end{exbox}
\K{Direkt}: liq.-wirksame Erträge $-$ liq.-wirksame Aufwände, je über Gegenkonto (z.B.\ Warenertrag $-$ Zunahme Ford.\ L+L $=$ Zahlung Kunden).
\textbf{Kalkulatorische Zinsen erscheinen im Cashflow nie.}\\
\K{Lückentechnik AV}: AB $-$ Abschr.\ $+$ Investition $-$ Desinvest.\ $=$ SB.\\
\K{Gesamte GFR}: CFO $+$ CFI $+$ CFF $=$ Veränderung flüssige Mittel
(\K{Liquiditätsnachweis}: FlüMi Anfang vs.\ Ende).\\
...
\K{Finanzierung} = Kapital \emph{beschaffen};
\K{Investition} = Mittel \emph{verwenden}.
Magisches Dreieck: Rendite–Sicherheit–Liquidität (+ Unabhängigkeit = Viereck).\\
\K{Mittelherkunft}: \emph{Aussenfin.} (extern: Kredit-=FK, Beteiligungs-=EK, Mezzanine)
vs.\ \emph{Innenfin.} (Selbst-, Abschreibungs-, Rückstellungsfin.).\\
\K{Finanzierungsvorgang}: Kapitalzuführung (verändert Bilanzsumme)
...
\sechead{Investitionsrechnung — Einführung}
